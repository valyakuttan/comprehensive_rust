...
///
/// You’ll start with some stubbed functions in an `impl` block as
/// well as a `User` struct definition. Your goal is to implement
/// the stubbed out methods on the `User` `struct` defined in the
/// `impl` block.
///
