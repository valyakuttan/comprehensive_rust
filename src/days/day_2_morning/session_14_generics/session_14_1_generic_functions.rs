...
///   generic function immediately, so that function must be valid for
///   all types matching the constraints. For example, try modifying
///   pick to return even + odd if n == 0. Even if only the pick
///   instantiation with integers is used, Rust still considers it
///   invalid. C++ would let you do this.
///
