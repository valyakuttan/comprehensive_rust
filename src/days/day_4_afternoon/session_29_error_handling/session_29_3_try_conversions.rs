...
/// the `?` operator on a function returning an `Option<AnyT>` or `Result<T, OtherErr>`
/// unless `OtherErr` implements `From<Err>`. Reciprocally, a function returning an
/// `Option<T>` can only apply the `?` operator on a function returning an `Option<AnyT>`.
///
...
///
/// It is good practice for all error types that don’t need to be `no_std` to implement
/// `std::error::Error`, which requires `Debug` and `Display`. The `Error` crate for
/// `core` is only available in nightly, so not fully `no_std` compatible yet.
///
/// It’s generally helpful for them to implement `Clone` and `Eq` too where possible,
/// to make life easier for tests and consumers of your library. In this case we
/// can’t easily do so, because `io::Error` doesn’t implement them.
///
