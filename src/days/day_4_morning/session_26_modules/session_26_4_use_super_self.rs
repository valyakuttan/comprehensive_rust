...
/// ```
///
/// Making `DiskStorage` and `NetworkStorage` available to other crates with
/// a convenient, short path.
///
/// For the most part, only items that appear in a module need to be `use`’d.
/// However, a trait must be in scope to call any methods on that trait, even
/// if a type implementing that trait is already in scope.
///
/// For example, to use the `read_to_string` method on a type implementing
/// the `Read` trait, you need to use `std::io::Read`.
///
